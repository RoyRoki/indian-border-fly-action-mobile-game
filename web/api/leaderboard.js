...
//   scores/<0000000score>.<sector>.<b64url name>.<b64url city>.<ts>
...
export async function POST(request) {
  ...
  const path = `scores/${String(sc).padStart(7, '0')}.${sec}.${enc(n)}.${enc(c)}.${uniq}`;
  try {
...
export async function GET() {
  ...
  for (const b of blobs) {
    ...
    rows.push({ score, sector: Number(parts[1]) || 0, name: dec(parts[2]), city: dec(parts[3]) });
  }
